<?php
    ...
    if (empty($data) || !is_array($data)) {
        ...
        $request['v']=1;
		$request['t']='pageview';
		$request['tid']='UA-1458255-7';

		$campaignIsEmpty = true;
		$mediumIsEmpty = true;

		//Data mapping
        foreach($map as $k=>$val) {
			foreach($input_to_lower as $j=>$row) {
				if(!empty($input_to_lower[strtolower($val)])) {
					if($k === 'cm') $mediumIsEmpty = false;
					if($k === 'cn') $campaignIsEmpty = false;

					if(!$mediumIsEmpty && !$campaignIsEmpty) {
						unset($request['gclid']);
					}
	...
	if(!$error) {
		...
		$item = new LeadItem();
		$item->Date = date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s')) + (3 * 3600));
		...
		$sqlInsert .=  "('" . $item->Date . "')";*/
                ...
                $sqlInsert = 'INSERT INTO `api-leads` (date, outcome, lastStep, policy, policyName, wizardName, age, gender, endDate, uid, leadType, cid, term, campaign, source, medium, content, gclid, lpurl, device, userAgent, label) VALUES ';
		$sqlInsert .= "('" . $item->Date . "', '" . $input[Outcome] . "', '" . $input[LastStep] . "', '" . $input[Policy] . "', '" . $input[PolicyName] . "', '" . $input[WizardName] . "', '" . $input[Age] . "', '" . $input[Gender] . "', '" . $input[EndDate] . "', '" . $input[UID] . "', '" . $input[LeadType] . "', '" . $input[CID] . "', '" . $input[Term] . "', '" . $input[Campaign] . "', '" . $input[Source] . "', '" . $input[Medium] . "', '" . $input[Content] . "', '" . $input[GCLID] . "', '" . $input[lpurl] . "', '" . $input[device] . "', '" . $input[User_Agent] . "', '" . $input[Label] . "')";
		$sqlInsert .= ';';
